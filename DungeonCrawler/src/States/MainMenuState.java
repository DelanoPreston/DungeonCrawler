...
//		exitImage.draw(exitX, exitY);
	...
	public void mouseClicked(int button, int x, int y, int clickCount) {
		...
		if (x >= playX && x <= playX + playImage.getWidth() && y >= playY && y <= playY + playImage.getHeight()) {
			playGame();
		} else if (x >= exitX && x <= exitX + exitImage.getWidth() && y >= exitY && y <= exitY + exitImage.getHeight()) {
			exitGame();
		}
	}
	...
	protected void playGame() {
		sbg.enterState(DungeonCrawler.GAMEPLAYSTATE);
	}
