...
public class AStarPathFinder {
	 ...
	 */
	public Path findPath(int type, int startX, int startY, int targetX, int targetY) {
		// easy first check, if the destination is blocked, we can't get there
		map.resetVisitedMap();
		if (map.blocked(type, targetX, targetY)) {
			return null;
		}
		...
		while ((maxDepth < maxSearchDistance) && (open.size() != 0)) {
			...
			for (int x = -1; x < 2; x++) {
				for (int y = -1; y < 2; y++) {
					...
					if (isValidLocation(type, startX, startY, xp, yp)) {
						// the cost to get to this node is cost the current plus
						// the movement

						// cost to reach this node. Note that the heuristic
						// value is only used

						// in the sorted open list

						float nextStepCost = current.cost + getMovementCost(type, current.x, current.y, xp, yp);
						...
						if (!inOpenList(neighbour) && !(inClosedList(neighbour))) {
							...
							neighbour.heuristic = getHeuristicCost(type, xp, yp, targetX, targetY);
	...
	public float getMovementCost(int type, int sx, int sy, int tx, int ty) {
		return map.getCost(type, sx, sy, tx, ty);
	}
	...
	public float getHeuristicCost(int type, int x, int y, int tx, int ty) {
		return heuristic.getCost(map, type, x, y, tx, ty);
	}
